// ローカル常駐版のエントリ。Bun で Hono の API とビルド済み SPA を配信する。
//
// データはローカルの SQLite ファイル (~/.track/track.db) に保存し、
// 認証なしの単一ユーザー用として動かす。
//
...
// src/server/index.ts からリポジトリルートを求める。
...
const migrated = runMigrations(DB_PATH, path.join(ROOT, MIGRATIONS_PATH));
