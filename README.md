...
Trackの起動中は、AIツール向けの接続情報を
`~/.track/runtime.json`へ自動的に書き出します。デスクトップ版が起動ごとに異なる
ポートを使っていても、連携コマンドはこのファイルからローカルAPIを検出できます。
ファイルには接続先とTrackプロセスのIDだけを保存し、パーミッションは所有者のみ
読み書き可能な`0600`に設定します。

Codex用スキルとClaude Code用`/track`コマンドは
`~/Workspace/ai-commands/track`にあります。どちらも現在のセッションから作業時間帯を
推定し、プロジェクト・タイトル・既存エントリとの重複を提示します。実際の登録は、
提示内容を利用者が明示的に確認した後にだけ行います。

- Codex: 「`$track`で今の作業を記録して」
- Claude Code: 「`/track`」

開発サーバーを`npm run local`で起動した場合も同じ仕組みで検出できます。
接続先を明示したい場合は`TRACK_API_BASE=http://127.0.0.1:8787/api`で上書きできます。
